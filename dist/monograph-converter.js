...
function configuredConverterBaseUrl() {
    const raw = process.env.WINDOWS_SERVER_URL ||
...
function endpointUrl(pathname) {
    ...
    if (!baseUrl) {
        throw new Error('Missing WINDOWS_SERVER_URL (or MONOGRAPH_CONVERTER_URL / NEXT_PUBLIC_WINDOWS_SERVER_URL) for HTML conversion.');
    }
...
async function bestEffortComplianceCheck(fetchFn, html) {
    try {
...
function tagForRow(row, bodyFontSize) {
    const len = row.text.length;
    if (row.avgFontSize >= bodyFontSize * 1.9 && len <= 120)
        return 'h1';
    if (row.avgFontSize >= bodyFontSize * 1.45 && len <= 180)
        return 'h2';
    return 'p';
}
function mergeItemsIntoRows(pageNum, items) {
    const atoms = items
        .map((item) => ({
